...
## Fix: Correção de Criação de Nós via TempNodeSlice (Optimistic UI)

**Identificador:** `fix_enhancement_tempNodeSlice_position_create`  
...
### 📝 Contexto
A funcionalidade de criar novos nós parou de funcionar corretamente. O método `invalidateQueries` está sendo usado de forma incorreta, forçando um *refetch* via GET que causa lentidão e instabilidade na interface.
